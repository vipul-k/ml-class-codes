...
def xavier(shape, seed=None):
    ...
    # todo initialize uniformly at random from [-sqrt(6/(n_in+n_out)), sqrt(6/(n_in+n_out))]
        lower_bound = -np.sqrt(6/(n_in + n_out))
        higher_bound = np.sqrt(6/(n_in + n_out))
        weights = np.random.uniform(low=lower_bound, high=higher_bound, size=shape)
...
class VDot:  # Matrix multiply (fully-connected layer)
    ...
    def backward(self):
        ...
        if self.b.grad is not None:
            ...
            #raise NotImplementedError
            self.b.grad  = []
            for i in range(self.b.value.shape[0]):                
                self.b.grad.append(self.grad*self.a.value[i])
            self.b.grad = np.array(self.b.grad)
...
class Sigmoid:
    ...
    def backward(self):
        if self.a.grad is not None:
            # todo
            #raise NotImplementedError
            self.a.grad = self.grad*(self.value**2)*np.exp(-self.a.value)
...
class RELU:
    ...
    def forward(self):
        # todo
        #raise NotImplementedError
        self.value = np.where(self.a.value <0 , 0, self.a.value)

    def backward(self):
        if self.a.grad is not None:
            # todo
            #raise NotImplementedError
            self.a.grad = self.grad*np.where(self.a.value <0 , 0, 1)
...
class SoftMax:
    ...
    def backward(self):
        if self.a.grad is not None:
            # todo
            #raise NotImplementedError
            self.a.grad = self.value*(self.grad - sum(self.grad*self.value))
...
class NN:
    ...
    def get_weights(self):
        ...
        for i in range(0,len(self.params) ,2):
            weights.append((self.params[i], self.params[i+1]))
        return weights
    ...
    def init_weights_with_xavier(self):
        ...
        for i in range(self.layer_number):
            w = xavier(shape = (self.nodes_array[i], self.nodes_array[i+1]),seed = i).astype(DATA_TYPE)
            b = np.zeros((self.nodes_array[i+1],)).astype(DATA_TYPE)
            weights.append((w, b))
    ...
    def build_computational_graph(self):
        ...
        for i in range(self.layer_number):
            # todo
            weight, bias = self.get_weights()[i]
    ...
    def sgd_update_parameter(self, lr):
        ...
        for item in self.params:
            item.value = item.value - item.grad*lr
        #raise NotImplementedError
